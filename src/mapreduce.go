...
var MyMapper func(value string) *list.List
var MyReducer func(key string, values *list.List) string
...
func InitMapReduce(nmap int, nreduce int,
	...
	Dlog.Printf("Step 4: Init map reduce params")
	mr := new(MapReduce)
	mr.nMap = nmap
	mr.nReduce = nreduce
	mr.file = file
...
func MakeMapReduce(nmap int, nreduce int,
	file string, master string) *MapReduce {
	Dlog.Println("Step 3: Making map reduce")
...
func (mr *MapReduce) StartRegistrationServer() {
	Dlog.Println("Step 5: Starting registration server")
	rpcs := rpc.NewServer()
	rpcs.Register(mr)
	...
	mr.l = l
	Dlog.Printf("Step 6: Now Accepting connections from registration server")
	...
	go func() {
		for mr.alive {
			...
			if err == nil {
				...
				DPrintf("RegistrationServer: accept error %w", err)
				break
			}
		}
...
func (mr *MapReduce) Run() {
	...
	Dlog.Println("Step 7: Splitting Files")
	mr.Split(mr.file)
	Dlog.Println("Step 8: Running master on split files")
	mr.stats = mr.RunMaster()
	Dlog.Println("Step 17: Outputting stats for finished jobs")
	for e := mr.stats.Front(); e != nil; e = e.Next() {
		Dlog.Println("Step 17: Jobs completed by worker:", e.Value)
	}
	Dlog.Println("Step 18: Merging documents")
	mr.Merge()
	Dlog.Println("Step 19: Cleaning up registration ")
	mr.CleanupRegistration()

	fmt.Printf("%s: MapReduce done\n", mr.MasterAddress)
	Dlog.Println("Step 20: Closing done channel to signify completion")
	mr.DoneChannel <- true
}
